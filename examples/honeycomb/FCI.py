...
def main():
    ...
    sym_rule = sym_U1 if args.sym == 'U1' else sym_Z2
    ...
    if args.instate is None or not os.path.exists(args.instate):
        if args.pattern == '1x1':
            ...
            stateAD = random_3x3_state_Z2(bond_dims=bond_dims, config=yastn_config) if args.sym == 'Z2' else \
            random_3x3_state_U1(bond_dims=bond_dims, config=yastn_config)
        else:

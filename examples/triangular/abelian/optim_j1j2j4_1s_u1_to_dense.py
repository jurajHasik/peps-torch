...
parser.add_argument("--tiling", default="1SITEQ", help="tiling of the lattice", \
    choices=["1SITEQ"])
...
def main():
    ...
    model= J1J2J4_1SITEQ_NOSYM(settings_full, j1=args.j1, j2=args.j2, j4=args.j4, jchi=args.jchi, \
            diag=args.diag)
    def energy_f(state,env):
        return model.energy_1x3(state,env,q=(0.5,0.5))
    def eval_obs_f(state,env):
        return model.eval_obs(state,env,q=(0.5,0.5))
    ...
    # TODO checkpointing
        ...
        state= IPEPS_ABELIAN(settings, dict(), lX=1, lY=1)

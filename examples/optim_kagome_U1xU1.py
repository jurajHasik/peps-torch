...
parser.add_argument("--phi", type=float, default=0., help="arctan(K/J): J -> 2-site coupling; K -> 3-site coupling")
parser.add_argument("--theta", type=float, default=0.5, help="arctan(H/K): K -> 3-site coupling; K -> chiral coupling")
...
def main():
    ...
    cfg.print_config()
    # Parametrization
    param_j = np.round(np.cos(np.pi*args.theta), decimals=15)
    param_k = np.round(np.sin(np.pi*args.theta) * np.cos(np.pi*args.phi), decimals=15)
    param_h = np.round(np.sin(np.pi*args.theta) * np.sin(np.pi*args.phi), decimals=15)
    ...
    if args.tiling=="BIPARTITE" or args.tiling=="2SITE" or args.tiling=="4SITE" \
        or args.tiling=="8SITE" or args.tiling=="1SITE":

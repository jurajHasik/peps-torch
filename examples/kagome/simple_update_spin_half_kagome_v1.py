...
def main():
    ...
    #observables
    energies=numpy.array(torch.zeros(len(itebd_list)), dtype='float64')
    m=numpy.array(torch.zeros(len(itebd_list)), dtype='complex128')
    if args.disp_corre_len:
        correl_len_x=numpy.array(torch.zeros(len(itebd_list)), dtype='float64')
        correl_len_y=numpy.array(torch.zeros(len(itebd_list)), dtype='float64')
    ...
    for ctt in range(len(itebd_list)):
        ...
        m[ctt]=(numpy.sqrt(obs_values[2])+numpy.sqrt(obs_values[3])+numpy.sqrt(obs_values[4]))/3
        if args.disp_corre_len:

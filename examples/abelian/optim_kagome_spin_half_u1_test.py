...
def main():
    ...
    unit_block= np.ones((1,1,1), dtype=args.GLOBALARGS_dtype)
    B_c1= yast.Tensor(settings_U1, s=(-1, 1, 1), n=0)
    B_c1.set_block(ts=(1,1,0), val= unit_block)
    B_c1.set_block(ts=(1,0,1), val= unit_block)
    B_c1.set_block(ts=(-1,-1,0), val= unit_block)
    B_c1.set_block(ts=(-1,0,-1), val= unit_block)
    B_b1=B_c1
    B_a1=B_c1
    ...
    unit_block= np.ones((1,1,1), dtype=args.GLOBALARGS_dtype)
    T_u1= yast.Tensor(settings_U1, s=(-1, -1, -1), n=0)
    T_u1.set_block(ts=(1,-1,0), val= unit_block)
    T_u1.set_block(ts=(-1,1,0), val= -1*unit_block)
    T_u1.set_block(ts=(0,1,-1), val= unit_block)
    T_u1.set_block(ts=(0,-1,1), val= -1*unit_block)
    T_u1.set_block(ts=(-1,0,1), val= unit_block)
    T_u1.set_block(ts=(1,0,-1), val= -1*unit_block)
    T_u1.set_block(ts=(0,0,0), val= unit_block)
    T_d1=T_u1
    state1= IPESS_KAGOME_GENERIC_ABELIAN(settings_U1, {'T_u': T_u1, 'B_a': B_a1, 'T_d': T_d1,\
                'B_b': B_b1, 'B_c': B_c1})


    B_c2 = yast.rand(config=settings_U1, s=(-1, 1, 1), n=0,
                    t=((-1, 1), (-1, 0, 1), (-1, 0, 1)),
                    D=((1, 1), (1, 1, 1), (1, 1, 1)))
    B_b2 = yast.rand(config=settings_U1, s=(-1, 1, 1), n=0,
                    t=((-1, 1), (-1, 0, 1), (-1, 0, 1)),
                    D=((1, 1), (1, 1, 1), (1, 1, 1)))
    B_a2 = yast.rand(config=settings_U1, s=(-1, 1, 1), n=0,
                    t=((-1, 1), (-1, 0, 1), (-1, 0, 1)),
                    D=((1, 1), (1, 1, 1), (1, 1, 1)))

    T_u2 = yast.rand(config=settings_U1, s=(-1, -1, -1), n=0,
                    t=((-1, 0, 1), (-1, 0, 1), (-1, 0, 1)),
                    D=((1, 1, 1), (1, 1, 1), (1, 1, 1)))
    T_d2 = yast.rand(config=settings_U1, s=(-1, -1, -1), n=0,
                    t=((-1, 0, 1), (-1, 0, 1), (-1, 0, 1)),
                    D=((1, 1, 1), (1, 1, 1), (1, 1, 1)))

    state2= IPESS_KAGOME_GENERIC_ABELIAN(settings_U1, {'T_u': T_u2, 'B_a': B_a2, 'T_d': T_d2,\
                'B_b': B_b2, 'B_c': B_c2})

    state3= read_ipess_kagome_generic(args.instate, settings_U1)

...
import logging
from models.toric_code import TORICCODE_2x2
import unittest
...
def main():
    ...
    model = TORICCODE_2x2(jv=args.jv,
                              jp=args.jp,
                              hx=args.hx,
                              hz=args.hz,
    ...
    if args.instate != None:
        ...
        sites = {(0, 0): A,(0, 1): A,(1, 0): A,(1, 1): A}
        state = IPEPS(sites)
    elif args.ipeps_init_type == 'TC_fixed_point':
        TA,TB=model.FPT_TC()
        sites = {(0, 0): TA.contiguous(),(0, 1): TB.contiguous(),(1, 0): TB.contiguous(),(1, 1): TA.contiguous()}
        state = IPEPS(sites)
        if args.bond_dim > max(state.get_aux_bond_dims()):
            state = extend_bond_dim(state, args.bond_dim)
        state.add_noise(noise=0.1)
        state.normalize_()
...
if __name__=='__main__':
    ...
    args.projector_eps_multiplet = 1.0e-8

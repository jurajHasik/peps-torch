...
def optimize_state(state, ctm_env_init, loss_fn, model, local_args, callbacks=[], \
    ...
    parameters= [state.parent_tensors[k] for k in state.parent_tensors.keys()]
    ...
    def closure():
        ...
        for i,k in enumerate(state.parent_tensors.keys()):
            state.parent_tensors[k]= parameters[i]
        # with torch.no_grad():
        # for c in state.parent_tensors.keys():
                # state.parent_tensors[c]= make_d2_symm(state.parent_tensors[c])
            # state.parent_tensors[c]= state.parent_tensors[c] + \
                    # state.parent_tensors[c].permute(0,1,4,3,2)

...
import torch
from optim import lbfgs_modified_NS
...
def optimize_state(state, ctm_env_init, loss_fn, obs_fn=None, post_proc=None,
    ...
    optimizer = lbfgs_modified_NS.LBFGS_MOD(parameters, max_iter=opt_args.max_iter_per_epoch, lr=opt_args.lr, \
    ...
    def closure(linesearching=False):
        ...
        if opt_args.opt_logging:
            ...
            if linesearching: log_entry["LS"]=len(t_data["loss_ls"])
                ...
                #sen niu: view replaced by reshape
                flat_grad= torch.cat(tuple(p.grad.reshape(-1) for p in parameters))

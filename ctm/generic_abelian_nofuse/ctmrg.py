...
from ctm.generic_abelian_nofuse.ctm_projectors import *
...
def run(state, env, conv_check=None, ctm_args=cfg.ctm_args, global_args=cfg.global_args): 
    ...
    for coord,A in state.sites.items():
        ...
        a._leg_fusion_data[0]= (0,1)
        a._leg_fusion_data[1]= (2,3)
        a._leg_fusion_data[2]= (4,5)
        a._leg_fusion_data[3]= (6,7)
        sitesDL[coord]=a
...
def absorb_truncate_CTM_MOVE_UP_c_nofuse(*tensors):
    ...
    #            --1,2 1,2--T1
    #                    2<-3(-1)
    nC1 = contract(Pt1, nC1, ([0,1,2],[0,1,2]))
    ...
    # T2--2,3 1,2--
    # 1->0(-1)
    nC2 = contract(nC2, P2, ([0,2,3],[0,1,2]))
    ...
    #            --0,1 2,3--A--6,7->4,5(-1)
    #                     4,5->2,3(-1)
    nT = contract(nT, A,([0,1,3,4],[2,3,0,1]))
...
def absorb_truncate_CTM_MOVE_LEFT_c_nofuse(*tensors):
    ...
    # 0      1,2
    # C2-----T2--3->1(-1)
    nC2 = contract(P2, nC2,([0,1,2],[0,1,2]))
    ...
    # |         0,1
    # |         0,1
    # T--4,5 2,3--A--6,7->4,5(-1)
    # 3->1(-1)  4,5->2,3(-1)
    nT= contract(nT, A,([0,1,4,5],[0,1,2,3]))
    ...
    # |___Pt2_|
    #     3(-1)
    nT = contract(nT, Pt2,([1,2,3],[0,1,2]))
...
def absorb_truncate_CTM_MOVE_DOWN_c_nofuse(*tensors):
    ...
    # |            |
    # C1--0 0------
    nC1 = contract(nC1, Pt1, ([0,2,3],[0,1,2]))
    ...
    #           |         3,4
    #            -----------T--5->1(-1)
    nT = contract(nT, A,([0,1,3,4],[2,3,4,5]))
    ...
    #        |      |           |
    #         ------T--1 0------
    nT = contract(nT, Pt2,([1,4,5],[0,1,2]))
    nT = nT.transpose((1,2,0,3))
...
def absorb_truncate_CTM_MOVE_RIGHT_c_nofuse(*tensors):
    ...
    #    (+1)2,3--A--6,7 3,4--T
    #       (+1)4,5    (-1)1<-5
    nT= contract(nT, A,([0,1,3,4],[0,1,6,7]))

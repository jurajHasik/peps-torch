...
def open_C2x2_LU(coord, state, env, fusion_level="full", verbosity=0):
    ...
    # |
    # 0
    # c2x2= c2x2.unfuse_legs_hard(axes=(1,2))
    ...
    if fusion_level=="full":
        # c2x2= c2x2.fuse_legs_hard(axes=(0,(1,2),3,(4,5),6,7))
        c2x2= c2x2.fuse_legs_hard( axes=((0,3),(1,4),2) )
    elif fusion_level=="basic":
        # c2x2= c2x2.fuse_legs_hard(axes=(0,(1,2),3,(4,5),6,7))
        pass
    else:
...
def open_C2x2_RD(coord, state, env, fusion_level="full", verbosity=0):
    ...
    # 3,4<-2,3<-2    | 
    #  5<-4<-3--T1---C
    # c2x2= c2x2.unfuse_legs_hard(axes=(1,2),inplace=True)
...
def rdm1x1(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    C2x1_LU= C2x1_LU.fuse_legs_hard( axes=((0,1),2) ) 
    ...
    rdm= rdm.unfuse_legs_hard(axes=0)
    if verbosity>0:
...
def rdm2x1(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    C2x1_LD= C2x1_LD.fuse_legs_hard(axes=((0,1),2))
    ...
    C2x1_RD= C2x1_RD.fuse_legs_hard(axes=((0,1),2))
    ...
    # 0123->0213
    # and normalize
    rdm= rdm.unfuse_legs_hard(axes=(0,1))
...
def rdm1x2(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    C1x2_RU= C1x2_RU.fuse_legs_hard(axes=((0,1),2))
    ...
    C1x2_RD= C1x2_RD.fuse_legs_hard(axes=(0,(2,1)))

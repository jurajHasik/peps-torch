...
# def reduced_density_matrix_kagome(rdm, phys_dim, site_types=('A', 'B', 'C')):
#     idp = torch.eye(phys_dim, dtype=rdm.dtype)
#     # The final two indices are physical indices!
#     dims_rdm = rdm.size()
#     reduced_phys_dim = dims_rdm[-1]
#     if phys_dim**3 != dims_rdm[-1]:
#         raise Exception("Physical dimensions do not agree. 1 site: {}, 1 unit cell: {}".format(phys_dim, dims_rdm[-1]))
#     rdm = rdm.view(*dims_rdm[:-2], phys_dim, phys_dim, phys_dim, phys_dim, phys_dim, phys_dim)
#     if 'A' in site_types:
#         if 'B' in site_types:
#             if 'C' in site_types:  # do nothing
#                 rdm = rdm.view(*dims_rdm[:-2], phys_dim ** 3, phys_dim ** 3)
#             else:  # trace out C
#                 reduced_phys_dim //= phys_dim
#                 rdm = torch.einsum('...ijkabc,kc->...ijab', rdm, idp).view(*dims_rdm[:-2], phys_dim ** 2, phys_dim ** 2)
#         else:
#             reduced_phys_dim //= phys_dim
#             if 'C' in site_types:  # trace out B
#                 rdm = torch.einsum('...ijkabc,jb->...ikac', rdm, idp).view(*dims_rdm[:-2], phys_dim ** 2, phys_dim ** 2)
#             else:  # trace out B & C
#                 reduced_phys_dim //= phys_dim
#                 rdm = torch.einsum('...ijkabc,jb,kc->...ia', rdm, idp, idp)
#     else:
#         reduced_phys_dim //= phys_dim
#         if 'B' in site_types:
#             if 'C' in site_types:  # trace out A
#                 rdm = torch.einsum('...ijkabc,ia->...jkbc', rdm, idp).view(*dims_rdm[:-2], phys_dim ** 2, phys_dim ** 2)
#             else: # trace out A & C
#                 reduced_phys_dim //= phys_dim
#                 rdm = torch.einsum('...ijkabc,ia,kc->...jb', rdm, idp, idp)
#         else:
#             reduced_phys_dim //= phys_dim
#             if 'C' in site_types:  # trace out A & B
#                 rdm = torch.einsum('...ijkabc,ia,jb->...kc', rdm, idp, idp)
#             else:  # trace out A & B & C -> physical dimension = 0
#                 reduced_phys_dim //= phys_dim
#                 rdm = torch.einsum('...ijkabc,ia,jb,kc->...', rdm, idp, idp, idp)
...
def build_reduced_density_matrix_kagome(coord, state, site_types=('A', 'B', 'C')):
    ...
    idp = torch.eye(phys_dim, dtype=state.dtype) / phys_dim
    if 'A' in site_types:
        if 'B' in site_types:
            if 'C' in site_types:  # do nothing
                ...
                rdm = einsum('miefgh,njabcd,ij->eafbgchdmn', tmp_site, conj(tmp_site), idp)
        else:
            reduced_phys_dim //= phys_dim
            if 'C' in site_types:  # trace out B
                ...
                reduced_phys_dim //= phys_dim
                tmp_site = state.site(coord).view(phys_dim, phys_dim, phys_dim, *dims_site[1:])
                rdm = einsum('mijefgh,nklabcd,ik,jl->eafbgchdmn', tmp_site, conj(tmp_site), idp, idp)
    else:
        reduced_phys_dim //= phys_dim
        if 'B' in site_types:
            if 'C' in site_types:  # trace out A
                tmp_site = state.site(coord).view(phys_dim, phys_dim**2, *dims_site[1:])
                rdm = einsum('imefgh,jnabcd,ij->eafbgchdmn', tmp_site, conj(tmp_site), idp)
            else:  # trace out A & C
                reduced_phys_dim //= phys_dim
                tmp_site = state.site(coord).view(phys_dim, phys_dim, phys_dim, *dims_site[1:])
                rdm = einsum('imjefgh,knlabcd,ik,jl->eafbgchdmn', tmp_site, conj(tmp_site), idp, idp)
        else:
            reduced_phys_dim //= phys_dim
            if 'C' in site_types:  # trace out A & B
                tmp_site = state.site(coord).view(phys_dim, phys_dim, phys_dim, *dims_site[1:])
                rdm = einsum('ijmefgh,klnabcd,ik,jl->eafbgchdmn', tmp_site, conj(tmp_site), idp, idp)
            else:  # trace out A & B & C -> physical dimension = 0
                reduced_phys_dim //= phys_dim
                tmp_site = state.site(coord).view(phys_dim, phys_dim, phys_dim, *dims_site[1:])
                rdm = einsum('ijkefgh,lmnabcd,il,jm,kn->eafbgchd', tmp_site, conj(tmp_site), idp, idp, idp)
                rdm = rdm.view(*rdm.size(), 1, 1)
...
def rdm1x1_kagome(coord, state, env, sites_to_keep=('A', 'B', 'C'), sym_pos_def=False, verbosity=0):
    ...
    Compute 1-kagome-site reduced density matrix :math:`\rho{1x1}_A` centered on vertex ``coord``.

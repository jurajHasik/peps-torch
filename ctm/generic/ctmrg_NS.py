...
def run(state, env, conv_check=None, ctm_args=cfg.ctm_args, global_args=cfg.global_args): 
    ...
    for i in range(ctm_args.ctm_max_iter):
        ...
        if conv_check is not None:
            ...
            # if i>2: 
            #     print(history[-1]-history[-2])
            #     spec_ers[0]=torch.linalg.vector_norm(spec1_new-spec1_old)
            #     spec_ers[1]=torch.linalg.vector_norm(spec2_new-spec2_old)
            #     spec_ers[2]=torch.linalg.vector_norm(spec3_new-spec3_old)
            #     spec_ers[3]=torch.linalg.vector_norm(spec4_new-spec4_old)

...
def rdm2x2_overlap(coord, state, state2, env, force_cpu=False, verbosity=0):
    r"""
    :param coord: vertex (x,y) specifies upper left site of 2x2 subsystem
    :param state: underlying wavefunction
    :param env: environment corresponding to ``state``
    ...
    if force_cpu:
        ...
        a_1layer = state.site(coord)
        a_1layer2 = state2.site(coord)
    dimsA1 = a_1layer.size()
    dimsA2 = a_1layer2.size()
    # contract all physical sites of this unit cell (index m)
    a = contiguous(einsum('mefgh,nabcd->eafbgchdmn', a_1layer, conj(a_1layer2)))
    a = view(a, (dimsA1[1] *dimsA2[1], dimsA1[2] *dimsA2[2], dimsA1[3] *dimsA2[3], dimsA1[4]*dimsA2[4],dimsA1[0], dimsA2[0]))
    ...
    C2x2_LU = view(C2x2_LU, (T2.size(1) * a.size(2), T1.size(2) * a.size(3), dimsA1[0], dimsA2[0]))
    if verbosity > 0:
        print("C2X2 LU " + str(coord) + "->" + str(state.vertexToSite(coord)) + " (-1,-1): " + str(C2x2_LU.size()))
    ...
    C2x2_RU = view(C2x2_RU, (T2.size(0) * a.size(1), T1.size(2) * a.size(2), dimsA1[0], dimsA2[0]))
    if verbosity > 0:
        print("C2X2 RU " + str((coord[0] + vec[0], coord[1] + vec[1])) + "->" + str(shitf_coord) + " (1,-1): " + str(
            C2x2_RU.size()))
    ...
    C2x2_RD = view(C2x2_RD, (T2.size(0) * a.size(0), T1.size(1) * a.size(1), dimsA1[0], dimsA2[0]))
    ...
    C2x2_LD = view(C2x2_LD, (T1.size(0) * a.size(1), T2.size(1) * a.size(1),dimsA1[0], dimsA2[0]))
    if verbosity > 0:
        print("C2X2 LD " + str((coord[0] + vec[0], coord[1] + vec[1])) + "->" + str(shitf_coord) + " (-1,1): " + str(
            C2x2_LD.size()))
    ...
    # C2x2_LD------C2x2_RD
    rdm = contract(upper_half, lower_half, ([0, 1], [0, 1]))
    rdm=rdm.view(dimsA1[0]*dimsA2[0],dimsA1[0]*dimsA2[0],dimsA1[0]*dimsA2[0],dimsA1[0]*dimsA2[0])
